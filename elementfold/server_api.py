...
from typing import Optional, List, Dict, Any     # ✴ readable type names
...
class HealthResponse:                              # ✴ /health
    ...
    version: str = "2.0.1"                         # project version string
...
def to_json(obj: Any) -> bytes:
    ...
    return json.dumps(obj, ensure_ascii=False).encode("utf-8")  # ✴ UTF‑8
...
def _as_int_list(x: Any) -> Optional[List[int]]:
    """Best‑effort convert to list[int] or None."""
    if x is None:
        return None
...
def _clean_strategy(s: Any) -> str:
    """Normalize strategy value; fall back to 'greedy'."""
    try:
        s = str(s).lower()
    except Exception:
        return "greedy"
...
def validate_infer_request(req: InferRequest) -> Optional[ErrorResponse]:
    ...
    Sanity checks:
      ...
      • sampling knobs only matter when strategy='sample'.
...
def resolve_tokens(req: InferRequest, tokenizer, vocab: int, seq_len: int) -> List[int]:
    ...
    limit = int(max(1, min(limit, seq_len)))                  # ✴ enforce a hard cap
...
def pack_infer_response(tokens_tensor, ledger_tensor, tokenizer=None) -> InferResponse:
    ...
    """
    # pick first batch row to keep API simple
    tokens = tokens_tensor[0].detach().cpu().tolist()
    ledger = ledger_tensor[0].detach().cpu().tolist()
