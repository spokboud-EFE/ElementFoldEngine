...
#        • AlignHead      — temperature‑free contrast to keep phases aligned on the δ⋆ circle.
#        • VariationalLedger — convex spacing energy that keeps seats/blocks evenly spaced.
#   2) We create an optimizer (AdamW groups) and a warmup→cosine LR schedule.
#   3) Each step we fetch a batch, run the model, compute three losses:
#        ℒ_task  (cross‑entropy),  ℒ_align  (contrast),  E_ledger (convex spacing),
#      and (optionally) a small **rung penalty** that encourages/avoids the rung band.
#   4) We backprop, clip gradients (safety), and step the optimizer/scheduler.
#   5) We measure coherence (κ), boundary pressure (p½), and **rung telemetry** (k, residuals).
#   6) We let the **RungController** fuse Supervisor hints with high‑level intent
#      (STABILIZE / HOLD / SEEK) and nudge β, γ, ⛔ on the model.
#
# Non‑expert map:
#   • β (beta)    — exposure (how strongly the gate exposes structure),
#   • γ (gamma)   — damping  (how hard normalization calms energy),
#   • ⛔ (clamp)  — gate cap  (how deep negative gate values are allowed before clipping),
#   • δ⋆          — the “click size” between rungs in the ledger coordinate X.
...
# Rung control: public, small, and always present.
from .rung_controller import RungController, RungIntent


# ————————————————————————————————————————————————————————————————————————
# Tiny UI helpers (Unicode‑aware with ASCII fallback)
# ————————————————————————————————————————————————————————————————————————
...
# ————————————————————————————————————————————————————————————————————————
# Rung helpers — compute k and residuals (with clear narration)
# ————————————————————————————————————————————————————————————————————————

def _rung_metrics(x: torch.Tensor, delta: float) -> Dict[str, torch.Tensor]:
    """
    Given a tensor of anchored‑log values X (any shape), compute three rung metrics:
      • k        = round(X / δ⋆)             (nearest integer rung),
      • r        = X − k·δ⋆                  (residual from the nearest rung center),
      • r_clicks = r / δ⋆                    (residual measured in “clicks” for readability).
    We do this element‑wise; callers can mean() to obtain batch‑level numbers.
    """
    d = float(delta)
    k = torch.round(x / d)
    r = x - k * d
...
def _rung_penalty(x_means: torch.Tensor, delta: float, band: float, intent: str) -> torch.Tensor:
    ...
    else:  # SEEK / disalign: inside the band is penalized so we sit safely away
...
# ————————————————————————————————————————————————————————————————————————
# Training loop — rung‑centric control + friendly logging
# ————————————————————————————————————————————————————————————————————————
...
def train_loop(
    ...
    rung_intent: str | RungIntent = RungIntent.STABILIZE,  # "stabilize" | "hold" | "seek"
    rung_target_k: int | None = None,  # target rung index for SEEK/HOLD strategies (optional)
    rung_band: float | None = None,    # acceptance half‑band in X; default = δ⋆/6 if None
    rung_loss_weight: float = 0.0,     # 0 = off; >0 adds a small rung penalty to total loss
):
    r"""
    ⟲ ElementFold training loop — **rung‑centric** and gentle.

    Why the RungController?
    -----------------------
    The Supervisor maintains coherence (β, γ, ⛔). The **RungController** then
    answers “*where on the click ladder should we sit?*”
      • STABILIZE — stay near the current rung (good for accuracy/consistency).
      • HOLD      — like stabilize, but with a tighter band (metrology‑grade runs).
      • SEEK      — politely stay *outside* the band (useful for safe disalignment).

    Acceptance “band”
    -----------------
    The acceptance half‑band defines the tolerance around a rung center (in X):
      band ≈ δ⋆ / 6 is a conservative default that leaves a healthy margin to the barrier.

    Optional rung penalty
    ---------------------
    You can let the *loss itself* “feel” rung behavior by setting `rung_loss_weight > 0`.
      STABILIZE/HOLD → penalize |r| outside the band,
      SEEK           → penalize |r| inside the band.
    This is gentle shaping; most steering still comes from β/γ/⛔ control.

    Unicode vs ASCII
    ----------------
    Output falls back to ASCII when the terminal lacks Unicode support.
    """
    ...
    for step in range(steps):
        ...
        # (optional) Rung penalty operates on batch‑means of X
        ...
        # Add friendly synonyms so downstream consumers are robust:
        tele["δ⋆"] = float(delta)
        if "kappa" in tele and "κ" not in tele:
            tele["κ"] = tele["kappa"]
        if "p_half" in tele and "p½" not in tele:
            tele["p½"] = tele["p_half"]
        ...
        ctrl_sup = sup.update(tele)                 # {'beta','gamma','clamp'} baseline
        ctrl_out = rung.update(tele, ctrl_sup)      # rung‑aware blend on top
