...
def _normalize_device(label: Optional[str]) -> Optional[str]:
    """Map arbitrary user labels → {'cuda','cpu',None(auto)}."""
    s=(label or "").strip().lower()
    if s in ("","auto","autodetect","default","none","null"): return None
    if s=="cpu": return "cpu"
...
def _normalize_intent(label:Optional[str])->str:
    ...
    aliases={
        "stabilise":"stabilize","stabilizer":"stabilize","stabilise/hold":"hold",
        "lock":"hold","center":"hold","step":"seek","seek":"seek","hold":"hold","stabilize":"stabilize"}
...
class Config:
    ...
    def to_kwargs(self)->Dict[str,Any]:
        ...
        return dict(
            device=self.device,steps=self.steps,vocab=self.vocab,d=self.d,
            layers=self.layers,heads=self.heads,seq_len=self.seq_len,fold=self.fold,delta=self.delta,
            capacities=self.capacities,batch=self.batch,use_data=self.use_data,
            lr=self.lr,wd=self.wd,warmup_frac=self.warmup_frac,clip_norm=self.clip_norm,tv_weight=self.tv_weight,
            out=self.out,print_every=self.print_every,ui=self.ui,
            rung_intent=self.rung_intent,rung_target_k=self.rung_target_k,
            rung_band=self.rung_band,rung_loss_weight=self.rung_loss_weight)
    ...
    def from_dict(cls,d:Dict[str,Any])->Config:
        ...
        if not isinstance(d,dict): raise TypeError(f"from_dict expects dict, got {type(d).__name__}")
    ...
    def from_json(cls,s:str)->Config:
        try: return cls.from_dict(json.loads(s))
        except Exception as e: raise ValueError(f"invalid JSON ({e})")

    @classmethod
    def load(cls,path:str,encoding:str="utf-8")->Config:
        with open(path,"r",encoding=encoding) as f: return cls.from_json(f.read())

    def save(self,path:str,indent:int=2,encoding:str="utf-8")->None:
        """Write pretty JSON; fall back to raw file if atomic writer missing."""
        try:
            from .utils.io import write_text
            write_text(path,self.to_json(indent))
        except Exception:
            with open(path,"w",encoding=encoding) as f: f.write(self.to_json(indent))
